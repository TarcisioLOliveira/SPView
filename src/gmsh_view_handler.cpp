...
GmshViewHandler::GmshViewHandler(const std::string& model_name, const std::string& view_name, size_t elem_num, size_t node_num, size_t mat_color_num, const ViewType view_type, const DataType data_type, defs::ModelType model_type, const size_t view_id)
...
void GmshViewHandler::update_view(const std::vector<double>& data, std::vector<size_t>& tags) const{
    ...
    } else if(this->view_type == NODAL){
        this->update_nodal(data, tags);
    } else if(this->view_type == TENSOR){
        this->update_tensor(data, tags);
    } else if(this->view_type == VECTOR){
